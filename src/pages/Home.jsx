...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/produk.json')
      ...
      .then(data => {
        setProduk(data)
        const katList = [...new Map(data.map(p => [p.kategori_id, { id: p.kategori_id, nama_kategori: p.nama_kategori }])).values()]
        setKategori(katList)
      })
  ...
  const kategoriLengkap = kategori.map(k => {
    ...
    const sample = produkKategori.find(p => p.gambar)
    return {
      ...k,
      count: produkKategori.length,
      sample_image: sample ? `/images/${sample.gambar}` : null
    }
  })
  ...
  return (
    <>
          ...
          {kategoriLengkap.length === 0 ? (
            ...
            <div className="kategori-list">
              {kategoriLengkap.map(k => (
                <Link key={k.id} to={`/katalog?kategori=${k.id}`} className="kategori-item">
                  <div className="kategori-item-image">
                    {k.sample_image ? (
                      <img src={k.sample_image} alt={k.nama_kategori} />
                    ) : (
                      <div className="kategori-item-noimg"><i className="fas fa-image"></i></div>
                    )}
                  </div>
      ...
      <section className="features">
        <div className="feature-item"><div>✂️</div><h4>Jahitan Rapi</h4><p>Dikerjakan dengan teliti oleh penjahit berpengalaman</p></div>
        <div className="feature-item"><div>🏆</div><h4>Bahan Premium</h4><p>Kualitas terbaik yang nyaman dan awet</p></div>
        <div className="feature-item"><div>🚚</div><h4>Pengiriman Cepat</h4><p>Dikirim ke seluruh Indonesia</p></div>
        <div className="feature-item"><div>💬</div><h4>Layanan Ramah</h4><p>Kami siap membantu kebutuhan Anda</p></div>
      </section>

      <section className="container" id="tentang">
        <div className="section-title"><h2>Tentang Kami</h2></div>
      ...
      <section className="features" id="kontak">
        <div style={{ maxWidth: '1600px', margin: 'auto', width: '100%' }}>
          <div className="section-title"><h2>Hubungi Kami</h2></div>

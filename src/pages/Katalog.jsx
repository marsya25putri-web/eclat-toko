...
export default function Katalog() {
  ...
  const produkFiltered = produk.filter(p => {
    ...
    const cocokKategori = kategoriAktif === 'semua' ||
      String(p.kategori_id) === String(kategoriAktif)
  ...
  return (
    <div className="container">
      ...
      <form
        ...
        style={{ maxWidth: '500px', margin: '0 auto 30px', display: 'flex' }}
      >
        <input
          type="text"
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            padding: '10px 22px',
            border: '1px solid #1a1a1a',
            background: kategoriAktif === 'semua' ? '#1a1a1a' : 'transparent',
            color: kategoriAktif === 'semua' ? 'white' : '#1a1a1a',
            fontSize: '0.7rem',
            letterSpacing: '2px',
        ...
        {kategori.map(k => {
          const jumlah = produk.filter(p => String(p.kategori_id) === String(k.id)).length
          return (
            <button
              key={k.id}
              ...
              style={{
                padding: '10px 22px',
                border: '1px solid #1a1a1a',
                background: String(kategoriAktif) === String(k.id) ? '#1a1a1a' : 'transparent',
                color: String(kategoriAktif) === String(k.id) ? 'white' : '#1a1a1a',
                fontSize: '0.7rem',
                letterSpacing: '2px',
                textTransform: 'uppercase',
                cursor: 'pointer',
      ...
      {(keyword || kategoriAktif !== 'semua') && (
        <p style={{ textAlign: 'center', color: '#6b6b6b', marginBottom: '40px', fontSize: '0.85rem', letterSpacing: '2px', textTransform: 'uppercase' }}>
          ...
          &nbsp;·&nbsp;
          <span onClick={handleReset} style={{ color: '#7a2e2e', textDecoration: 'underline', cursor: 'pointer' }}>
            Reset
          </span>
        </p>
      )}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '80px 20px' }}>
          ...
          <p style={{ color: '#6b6b6b', marginTop: '15px' }}>Coba kata kunci atau kategori lain.</p>
        </div>
      ) : (
            ...
            <div className="produk-card" key={p.id}>
              <div className="image-wrapper">
                {p.gambar_url ? (
                  ...
                  <div style={{ width: '100%', height: '380px', background: '#f5f3f0', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#d4d4d4', letterSpacing: '2px', fontSize: '0.8rem' }}>NO IMAGE</div>
                )}
              </div>
